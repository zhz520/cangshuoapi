...
exports.sendEmailToAdmin = (req, res) => {
  ...
  db.query(sql, ip, (err, results) => {
    ...
    if (results.length === 0) {
      db.query(
        "insert into mail_record set ?",
        { ip: ip, last_sendmail_date: new Date() },
        (err, results) => {
          if (err) return res.cc(err);
    ...
    if (results.length > 0) {
      ...
      if (diff >= 1000 * 60 * 60 * 24) {
        db.query(
          "update mail_record set last_sendmail_date=? where ip=?",
          [new Date(), ip],
          (err, results) => {
            if (err) return res.cc(err);
            sendMail(res, mailOptions);
...
exports.getLanZouLink = async (req, res) => {
  ...
  if (!pwd || pwd == null) {
    try {
      ...
      if (!iframeSrc)
        return res.status(400).json({ status: 1, message: "解析失败！" });
...
async function getLanZouLink(url, urlObj, res, type) {
  ...
  try {
    ...
    if (type === "json" || type === "JSON" || type === null || !type) {
      res
        .status(200)
        .json({ status: 0, message: "解析成功！", data: { urlLink } });
    } else if (type === "down") {
...
async function getKuaishouLinks(urlPFull, type, url, res) {
  try {
    ...
    const imageUrlq = `${
      chenzongtechHtml.match(/"coverUrls"\s*:\s*\[([^\]]+)\]/)[1]
    }`.split(",")[1];
    ...
    if (index !== -1) {
      let result = chenzongtechHtml.substring(
        index + substring.length + 2,
        chenzongtechHtml.length - 2
      );
      ...
      if (type === "json" || type === "JSON" || type === null || !type) {
        res.status(200).json({
          status: 0,
          message: "解析成功！",
          data: { urlLink, author, imageUrls, caption },
        });
      } else if (type === "down") {
        res.redirect(302, urlLink);
      } else {
...
function extractUrls(html) {
  const regex =
    /"play_addr"\s*:\s*\{\s*"uri"\s*:\s*"([^"]+)"\s*,\s*"url_list"\s*:\s*\[\s*"([^"]+)"(?:\s*,\s*"([^"]+)")?\s*\]\s*\}/;
  ...
  try {
    ...
    if (type === "json" || type === "JSON" || type === null || !type) {
      res.status(200).json({
        status: 0,
        message: "解析成功！",
        data: { urlLink: videoUrl, author, desc, imgUrl },
      });
    } else if (type === "down") {
    ...
    res
      .status(500)
      .json({ status: 1, message: "解析失败！", error: error.message });
  }
};
// 抖音结束
...
exports.getQQAuthor = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: 0, message: "解析成功！", data: { author: authorQQ } });
  } catch (error) {
...
const openId = "oU" + generateRandomString(5);
const stationId = Math.floor(5000 + Math.random() * 50000).toString();
// 三要素
exports.getThreeElements = async (req, res) => {
  const { realName, phone, cardNo } = req.query;

  // 验证输入参数
  if (!realName || !phone || !cardNo) {
    return res.status(400).json({ status: 1, message: "请输入三要素！" });
  }

  const url = "https://gxhs.wtkj.site/dev-api/gxhs/front/make_auth";
  const headers = {
    "Content-Type": "application/json",
    "User-Agent":
      "Mozilla/5.0 (iPhone; CPU iPhone OS 17_5_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.53(0x180032e) NetType/WIFI Language/zh_CN",
  ...
  try {
    ...
    console.error(`Error in getThreeElements: ${error.message}`);
